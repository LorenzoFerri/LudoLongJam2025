...
func build_description() -> String:
	...
	if percentageValue > 0:
		result += "Reduce fuel lost on hit by [color={1}]{0}[/color]%.".format([percentageValue, UpgradeUtilsClass.get_color(percentageValue)])
	elif percentageValue < 0:
		result += "Increase fuel lost on hit [color={1}]{0}[/color]%.".format([percentageValue, UpgradeUtilsClass.get_color(percentageValue)])
