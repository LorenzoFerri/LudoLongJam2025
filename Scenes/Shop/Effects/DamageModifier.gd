...
func build_description() -> String:
	...
	if percentageValue > 0:
		result += "Increase damage by [color={1}]{0}[/color]% per hit.".format([percentageValue, UpgradeUtils.get_color(percentageValue)])
	elif percentageValue < 0:
		result += "Decrease damage by [color={1}]{0}[/color]% per hit.".format([percentageValue, UpgradeUtils.get_color(percentageValue)])
