...
func build_description() -> String:
	...
	if percentageValue > 0:
		result += "Increase maximum speed by [color={1}]{0}[/color]%.".format([percentageValue, UpgradeUtils.get_color(percentageValue)])
	elif percentageValue < 0:
		result += "Decrease maximum speed by [color={1}]{0}[/color]%.".format([percentageValue, UpgradeUtils.get_color(percentageValue)])
